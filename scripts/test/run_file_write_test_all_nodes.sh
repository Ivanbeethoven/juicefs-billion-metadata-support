...
ENV_TARGET_FILES_PER_NODE="${TARGET_FILES_PER_NODE:-}"
EXPLICIT_TARGET_FILES_PER_NODE="${FILE_WRITE_TARGET_PER_NODE:-}"
TARGET_FILES_PER_NODE=""
...
if [ -n "$EXPLICIT_TARGET_FILES_PER_NODE" ]; then
  TARGET_FILES_PER_NODE="$EXPLICIT_TARGET_FILES_PER_NODE"
elif [ -n "$FILE_WRITE_TOTAL_FILES" ]; then
  TARGET_FILES_PER_NODE=$(( (FILE_WRITE_TOTAL_FILES + host_count - 1) / host_count ))
elif [ -n "$ENV_TARGET_FILES_PER_NODE" ]; then
  TARGET_FILES_PER_NODE="$ENV_TARGET_FILES_PER_NODE"
else
